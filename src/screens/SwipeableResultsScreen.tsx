...
export default function SwipeableResultsScreen({ imageUri, results, amRoutine, pmRoutine, onTryAgain, onContinue }: SwipeableResultsScreenProps) {
  // Early return safety check to prevent hooks violation - MUST be before any hooks
  if (!results || !imageUri) {
    return (
  ...
  const totalPages = 3; // Ratings, Concerns, Skincare Routine (COMMENTED OUT: Skin Goals)
  ...
  // const [isLocked, setIsLocked] = useState(Platform.OS === 'android' ? false : true); // Android always unlocked, iOS paywall enabled by default
  ...
  // Vertical slide for swipe-down dismiss on page 3 (currentPage === 2)
  ...
  // COMMENTED OUT: Render Your Skin Goals page (page 3) - Perfect Skin Image Generation
          ...
          <View style={styles.fullProductSection}>
            ...
            <View style={styles.fullProductInfo}>
              ...
              <View style={styles.fullProductDetails}>
                <Text style={styles.fullProductName}>{step.productType}</Text>
                <Text style={styles.fullProductType}>{step.productName || step.productType}</Text>
              </View>
            </View>
  ...
  // Page 3: Full Skincare Routine (now the final page)
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Wrap entire content including nav bar in animated view for page 3 (currentPage === 2) */}
      <Animated.View 
        style={[
          styles.content, 
          currentPage === 2 ? { transform: [{ translateY: slideY }] } : {}
        ]}
      >
        {Platform.OS === 'android' ? (
          <View style={styles.swipeContainer}>
            ...
            >
              {renderCompactPage()}
              {renderConcernsPage()}
              {/* COMMENTED OUT: {renderSkinGoalsPage()} */}
  ...
  >
    ...
    <Text style={styles.unlockButtonText}>Unlock</Text>
        ...
        <View style={styles.bottomNavBar}>
          ...
          {/* Continue Button - only show on last page (3rd screen - Skincare Routine) and when unlocked */}
